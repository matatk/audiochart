...
!function() {
    var DataWrapper = function() {
        function DataWrapper(data) {
            throw this.data = data, Error("Please use a derived class");
        }
        ...
        return FrequencyPitchMapper.prototype = Object.create(PitchMapper.prototype), FrequencyPitchMapper.prototype.map = function(datum) {
            var ratio;
        ...
        return WebAudioSounder.prototype.start = function() {
            ...
            }(this), setTimeout(callback, offset);
        ...
        function Player(duration, data, pitch_mapper, sounder, visual_callback) {
            this.data = data, this.pitch_mapper = pitch_mapper, this.sounder = sounder, this.visual_callback = null !== visual_callback ? visual_callback : null, 
        ...
        function AudioChart(options, context) {
            ...
            if (null === context && (context = null), null === context && (context = AudioContextGetter.get(), 
            null === context)) throw Error(fail);
        ...
        var audio_context = null, _get_audio_context = function() {
            if ("undefined" != typeof window && null !== window) {
                if (null !== window.AudioContext) return new window.AudioContext();
                if (null !== window.webkitAudioContext) return new window.webkitAudioContext();
            }
            return null;
        };
    ...
    window.AudioChart = AudioChart, window._AudioChart = _AudioChart, window.AudioContextGetter = AudioContextGetter, 
    window.DataWrapper = DataWrapper, window.GoogleDataWrapper = GoogleDataWrapper, 
    window.JSONDataWrapper = JSONDataWrapper, window.HTMLTableDataWrapper = HTMLTableDataWrapper, 
    window.PitchMapper = PitchMapper, window.FrequencyPitchMapper = FrequencyPitchMapper, 
    window.NotePitchMapper = NotePitchMapper, window.WebAudioSounder = WebAudioSounder, 
    window.Player = Player, window.google_visual_callback_maker = google_visual_callback_maker, 
    window.html_table_visual_callback_maker = html_table_visual_callback_maker, alert(window.AudioChart);
}();

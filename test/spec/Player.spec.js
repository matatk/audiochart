...
var expected_frequency_calls = function(playback_time, series_length) {
	...
	out = []
	out.push([21])
	for (i = j = 1, ref = series_length - 1; ref >= 1 ? j <= ref : j >= ref; i = ref >= 1 ? ++j : --j) {
...
var mixin_data_wrapper_core = function(message, TestDataClass, test_duration, test_call_count, test_interval, use_visual_callback) {
	describe(message, function() {
		var fake_data = null
		var fake_mapper = null
		var fake_sounder = null
		var player = null
		var fake_visual_callback = null  // may not be used

		beforeEach(function() {
			fake_data = new TestDataClass()
...
var mixin_data_wrapper = function(message, TestDataClass, test_duration, test_call_count, test_interval) {
	describe(message, function() {
		mixin_data_wrapper_core('when not having a callback', TestDataClass, test_duration, test_call_count, test_interval, false)
		mixin_data_wrapper_core('when having a callback', TestDataClass, test_duration, test_call_count, test_interval, true)
	})
}
